...
# Reproject lat/lon to UTM, then create a running distance along the section
# in km
from pyproj import Transformer, CRS
xformer = Transformer.from_crs(CRS.from_epsg(4326), CRS.from_epsg(32610),
        always_xy=True)
xys = np.array([xformer.transform(lon, lat) for lon, lat in zip(ds['lon'][:], ds['lat'][:])])
legs = np.concatenate(([0], np.sqrt((xys[1:,0] - xys[:-1,0]) ** 2 +
    (xys[1:,1] - xys[:-1,1]) ** 2)))
xsect = np.cumsum(legs) / 1000
